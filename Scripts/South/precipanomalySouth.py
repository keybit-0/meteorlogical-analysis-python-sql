...
for y in range(1981, 2021):
        ...
        sql = 'SELECT sum(precip) AS totalprecip FROM Project.AllData WHERE YEAR(date_time) =' + str(y) + ' and hour(date_time) = 23 and location = "S" ORDER BY id;'
...
#data.to_pickle("SouthprecipanomYear+bestfit.pkl")
print(data)
...
plt.title('Annual Precipitation Anomaly in South Ireland')
plt.xlabel('Year')
