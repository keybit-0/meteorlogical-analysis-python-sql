...
for m in range(1,13):
    ...
    sql = 'SELECT AVG(temp) as avgtemp FROM Project.AllData where month(date_time) =' + str(m) + ' and location = "S";'
...
for y in range(1981, 2021):
    sql = 'SELECT AVG(temp) as avgtemp FROM Project.AllData where year(date_time) =' + str(y) + ' and location = "S" order by id;'
...
with open('baselinetempS.pickle', 'wb') as handle:
    pickle.dump(baselinetempS, handle, protocol=pickle.HIGHEST_PROTOCOL)

# Opens file and print baseline values. 
with open('baselinetempS.pickle', 'rb') as handle:
