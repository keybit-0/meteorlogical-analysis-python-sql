...
for y in range(1981, 2021):
    ...
    for m in range(1,13):
        ...
        sql = 'SELECT AVG(temp) as avgtemp FROM Project.AllData where month(date_time) =' + str(m) + ' and year(date_time) =' + str(y) + ' and location = "S" order by id;'
...
data.to_pickle("SouthTempanomMonth.pkl")
