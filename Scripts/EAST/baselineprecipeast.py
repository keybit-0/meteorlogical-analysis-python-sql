...
for m in range(1,13):
    ...
    sql = 'SELECT SUM(precip) as totalprecip FROM Project.AllData where month(date_time) =' + str(m) + ' and hour(date_time) = 23 and location = "E";'
    ...
    baselineprecipE[m]=(monthavg[0][0])/(len(range(1981, 2021)))
...
sql = 'SELECT SUM(precip) as totalprecip FROM Project.AllData where hour(date_time) = 23 and location = "E" order by id;'
mycursoryear.execute(sql)
result = mycursoryear.fetchall()

# add average annual temp to baseline West dictionary
yearavg = (result[0][0])/(len(range(1981, 2021)))
baselineprecipE['year']= yearavg
...
with open('baselineprecipE.pickle', 'wb') as handle:
    pickle.dump(baselineprecipE, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('baselineprecipE.pickle', 'rb') as handle:
