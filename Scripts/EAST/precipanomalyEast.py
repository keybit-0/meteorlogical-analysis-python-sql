...
for y in range(1981, 2021):
        ...
        # Sql query for total annual precip .
        sql = 'SELECT sum(precip) AS totalprecip FROM Project.AllData WHERE YEAR(date_time) =' + str(y) + ' and hour(date_time) = 23 and location = "E";'

        # Subtract the annual precip from baseline precip and append to anomaly.
...
#store in .pkl file
data.to_pickle("EastprecipanomYear+bestfit.pkl")
...
plt.title('Annual Precipitation Anomaly in East Ireland')
plt.xlabel('Year')
