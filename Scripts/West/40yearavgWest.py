...
for m in range(1,13):
    ...
    sql = 'SELECT AVG(temp) as avgtemp FROM Project.AllData where month(date_time) =' + str(m) + ' and location = "W";'
...
for y in range(1981, 2021):
    sql = 'SELECT AVG(temp) as avgtemp FROM Project.AllData where year(date_time) =' + str(y) + ' and location = "W" order by id;'
...
with open('baselinetempW.pickle', 'wb') as handle:
    pickle.dump(baselinetempW, handle, protocol=pickle.HIGHEST_PROTOCOL)
